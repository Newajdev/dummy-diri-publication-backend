...
const routeParam = (value: string | string[] | undefined, name: string): string => {
  if (typeof value !== "string" || !value) throw new AppError(400, `Invalid ${name} parameter.`);
  return value;
};
export class ArticleController {
  ...
  async getBySlug(req: Request, res: Response): Promise<void> { const slug = routeParam(req.params.slug, "slug"); res.json({ success: true, message: "Article retrieved successfully.", data: await service.bySlug(slug) }); }
  async remove(req: Request, res: Response): Promise<void> { const id = routeParam(req.params.id, "id"); await service.remove(id); res.json({ success: true, message: "Article deleted successfully.", data: {} }); }
}
